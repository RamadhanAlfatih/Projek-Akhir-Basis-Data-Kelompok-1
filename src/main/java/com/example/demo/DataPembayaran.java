...
public class DataPembayaran implements Initializable {
    ...
    @FXML
    private TableColumn<Pembayaran, Integer> idpegawaiCol;
    ...
    @FXML
    private TableColumn<Pembayaran, Integer> nocucianCol;

    @FXML
    private TextField nocucianText;

    @FXML
    private TableColumn<Pembayaran, Integer> nopembayaranCol;
    ...
    private void clickTabel() throws IOException {
        ...
        nopembayaranText.setText(String.valueOf(nopembayaranCol.getCellData(index)));
    ...
    public void Update() {
        ...
        nocucianCol.setCellValueFactory(new PropertyValueFactory<Pembayaran, Integer>("NomorCucian"));
        idpegawaiCol.setCellValueFactory(new PropertyValueFactory<Pembayaran, Integer>("IdPegawai"));
    ...
    public static ObservableList<Pembayaran> getDatausers() {
        ...
        try {
            ...
            while (result.next()) {
                list.add(new Pembayaran(result.getInt("NoPembayaran"), result.getString("JenisPembayaran"), result.getFloat("TotalHarga"),
                        result.getInt("NomorCucian"), result.getInt("IdPegawai")));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void cariData() {
        ...
        cariText.textProperty().addListener((observableValue, oldValue, newValue) -> filteredData.setPredicate(person -> {
            ...
            if (String.valueOf(person.getNoPembayaran()).toLowerCase().contains(lowerCaseFilter)) {
                return true;
            } else if (person.getJenisPembayaran().toLowerCase().contains(lowerCaseFilter)) {
                return true;
            } else if (String.valueOf(person.getNomorCucian()).toLowerCase().contains(lowerCaseFilter)) {
                return true;
            } else if (String.valueOf(person.getIdPegawai()).toLowerCase().contains(lowerCaseFilter)) {
                return true;
    ...
    public void tambahPembayaran() {
        DatabaseConnection connectNow = new DatabaseConnection();
        Connection connectDB = connectNow.getConnection();
        String query = "Insert into Pembayaran (JenisPembayaran, TotalHarga, NomorCucian, IdPegawai) values (?,?,?,?)";
        try {
            PreparedStatement sqlStatement = connectDB.prepareStatement(query);
            sqlStatement.setString(1, jenispembayaranText.getText().trim());
            sqlStatement.setString(2, totalhargaText.getText().trim());
            sqlStatement.setString(3, nocucianText.getText());
            sqlStatement.setString(4, idpegawaiText.getText().trim());
    ...
    public void editPembayaran() {
        try {
            ...
            String query = "Update Pembayaran Set JenisPembayaran='" + value2 + "',TotalHarga='" + value3 + "',NomorCucian='" + value4 + "',IdPegawai='" +
